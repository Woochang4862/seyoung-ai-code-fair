...
def draw_points(image, landmarks, indices, color=(0, 255, 0), label=True):
    """
    지정한 랜드마크 번호만 점으로 화면에 그린다.

    Parameters
    ----------
    landmarks : result.face_landmarks[0] — NormalizedLandmark 리스트
    indices   : 그릴 랜드마크 번호 목록
    """
    h, w = image.shape[:2]
    for idx in indices:
        lm = landmarks[idx]
    ...
    with FaceLandmarker.create_from_options(options) as landmarker:
        while cap.isOpened():
            ...
            if result.face_landmarks:
                ...
                # 오른쪽 눈 → 초록
                draw_points(frame, lm, RIGHT_EYE, color=(0, 255, 0))
                # 왼쪽 눈 → 파랑
                draw_points(frame, lm, LEFT_EYE,  color=(255, 100, 0))
